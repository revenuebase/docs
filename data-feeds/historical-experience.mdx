...
| `linkedin_id` | INTEGER | LinkedIn company page ID (from organisation table) |
...
<AccordionGroup>
  ...
  <Accordion title="Company alumni targeting">
    Find everyone who used to work at a specific company by flattening both arrays and filtering for `experience.job_org_linkedin_url` matching the target company, or use `jobs[].rbid_org` to join to the organisation table. Filter for `experience.job_is_current = FALSE`. Useful for selling to former champions who already know your space.
  </Accordion>
